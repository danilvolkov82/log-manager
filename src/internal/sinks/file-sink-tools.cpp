...
fs::path 
makeBackupPath(const fs::path &current_file, std::uint32_t index) {
    return fs::path(current_file.string() + "." + std::to_string(index));
}

bool
tryMoveFile(const fs::path &source, const fs::path &target) {
    std::error_code ec;
#ifdef _WIN32
    // TODO: Replace with WinAPI-based move/replace (e.g. MoveFileExW) with lock-aware retries.
    // Current remove+rename flow is simple but can still lose the previous target on rename failure.
...
std::vector<BackupFile>
collectBackups(const fs::path &current_file) {
    ...
    const std::regex file_template(R"(^(.+)\.(\d+)$)");
    ...
    while(it != end) {
        ...
        std::smatch match;
        if (!std::regex_search(file_name, match, file_template)) {
            continue;
        }

        if(base_name.compare(match[1]) != 0) {
            continue;
        }

        const std::string index_text = match[2].str();
        std::uint32_t index = 0;
        const char *begin = index_text.data();
        const char *end_ptr = begin + index_text.size();
        const auto parse_result = std::from_chars(begin, end_ptr, index);
        if(parse_result.ec != std::errc() || parse_result.ptr != end_ptr || index == 0) {
            continue;
        }
...
void
pruneBackups(const fs::path &current_file, std::int32_t max_backup_files) {
...
std::uint64_t
getFileSize(const fs::path &file_path) {
    std::error_code ec;
...
fileTimeToSystemClock(fs::file_time_type file_time) {
...
int
toLocalDateKey(std::chrono::system_clock::time_point timestamp) {
...
bool
tryToLocalDateKey(std::chrono::system_clock::time_point timestamp, int &date_key) {
    try {
...
bool
isOlderThanDays(const fs::path &file_path, std::uint32_t max_file_age_days, std::chrono::system_clock::time_point now) {
...
std::string
tokenToRegex(std::string_view token) {
    if(token == "yyyy") {
        return R"(\d{4})";
    }
...
std::regex
buildFilenameRegex(const std::string &filename_template) {
...
void
rotateFile(const fs::path &current_file, std::int32_t max_backup_files) {
    std::error_code ec;
    if(max_backup_files == 0) {
    ...
    for(const auto &backup : backups) {
        ...
        if(!tryMoveFile(backup.path, target)) {
            return;
        }
    }

    fs::path first_backup = makeBackupPath(current_file, 1);
    if(!tryMoveFile(current_file, first_backup)) {
        return;
    }
...
void
ensureParentDirectory(const fs::path &current_file) {
...
bool
shouldRotateBySize(const fs::path &current_file, std::uint64_t incoming_bytes, std::uint64_t max_file_size) {
    if(max_file_size == 0) {
        return false;
    }
...
bool
shouldRotateByDaily(const fs::path &current_file, std::chrono::system_clock::time_point entry_timestamp) {
    std::error_code ec;
...
void
pruneTemplateFilesByAge(const fs::path &current_file, const std::string &filename_template, std::uint32_t max_file_age_days) {
    if(max_file_age_days == 0) {
        return;
    }
...
void
appendLine(const fs::path &current_file, const std::string &line) {
